...
class Processor:
    def __init__(self, data_tag, f_rgb, f_lidar, data_dir, aug):
    ...
    def __call__(self,load_index):
        if self.aug:
            ...
            nusc_mode = True
            if nusc_mode:
                raw_lidar = np.fromfile(self.f_lidar[load_index], dtype=np.float32).reshape((-1, 5))
                raw_lidar = raw_lidar[:, :4]
                raw_lidar = raw_lidar[:, [1,0,2,3]] # change nusc lidar coord to kitti
                raw_lidar[:, 1] = -raw_lidar[:, 1]
                mask = raw_lidar[:, 0] > 2.5 # remove ego points in nusc
                raw_lidar = raw_lidar[mask]
            else:
...
def iterate_data(data_dir, shuffle=False, aug=False, batch_size=1, multi_gpu_sum=1):
    f_rgb = glob.glob(os.path.join(data_dir, '*.png'))
    f_lidar = glob.glob(os.path.join(data_dir, '*.bin'))
    f_rgb.sort()
    f_lidar.sort()
    ...
    proc=Processor(data_tag, f_rgb, f_lidar, data_dir, aug)
